...
There are also `example-plugin/` and `example-rust-runtime/` directories, which are compiled against
the bindings generated by the `example-protocol`. This way, you can see how the bindings are
expected to be used, and we verify that the generated bindings actually compile.
